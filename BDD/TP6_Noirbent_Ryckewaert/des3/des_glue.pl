...
%  MyOptions = [open(once),cursor_type(dynamic),silent(true)|Options],
  ...
  (odbc_connect(Connection,Handle,MyOptions)
   ->
    assertz(opened_db(Connection,(Handle,MyOptions),_)),
    my_odbc_get_dbms(Connection,DBMS),
    retract(opened_db(Connection,(Handle,MyOptions),_)),
    (DBMS == sqlserver,
...
%      retract(opened_db(Connection,_,_)),
      assertz(opened_db(Connection,(MarsHandle,[SQLServerOption|MyOptions]),DBMS))
...
%       odbc_set_connection(Handle,SQLServerOption)
     ;
      assertz(opened_db(Connection,(Handle,MyOptions),DBMS))
    ),
...
my_close_odbc(Connection) :-
  (retract(opened_db(Connection,(Handle,_Options),_DBMS))
   ->
    set_default_db,
    odbc_disconnect(Handle),
...
my_odbc_dql_query_wo_schema(SQLstr,Rows) :-  
  current_db(Connection),
  my_odbc_dql_query(Connection,SQLstr,Rows).
...
my_odbc_dangling_query_close :-
  current_db(Connection),
  retract(opened_db(Connection,(Handle,Options),DBMS)),
  !,
  odbc_disconnect(Handle),
  odbc_connect(Connection,NHandle,Options),
  assertz(opened_db(Connection,(NHandle,Options),DBMS)).
my_odbc_dangling_query_close.
