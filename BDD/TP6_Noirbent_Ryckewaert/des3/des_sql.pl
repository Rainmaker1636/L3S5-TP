...
% The following is automatically added in des.pl at start-up:
% % Tables
...
where_conditions(Connection,[Col|Cols],[Name|Names],[CondStr|CondStrList]) :-
  ...
  (Col = '$NULL'(_)
   ->
    StrComp = " IS ", ValStr="NULL"
   ;
    StrComp = "=", quote_value(Col,ValStr)
  ),
...
:- dynamic(last_syntax_error/2).  % Last syntax error from parsing
%last_syntax_error(_NoError).
...
my_DDL(CRVSchema) -->
  ...
  my_sql_blanks,
  push_syntax_error(['Missing AS after CREATE VIEW'],Old),
  my_kw("AS"),
  my_sql_blanks,
...
my_DDL(CRVSchema) -->
  my_complete_constrained_typed_schema(Schema,_), % No constraints
  my_sql_blanks_star,
  ":",
  my_sql_blanks_star,
  "=",
  my_sql_blanks_star,
  my_DQL((SQLst,Schema)),
  {CRVSchema =.. [create_or_replace_view,sql,(SQLst,_AS),Schema]}.
...
remaining_chars(R,I,I) :-
  length(I,R).

set_syntax_error(M,L) :-
  (last_syntax_error(_M,LL),
   LL>L
   ->
    set_flag(last_syntax_error(M,L))
   ;
    true).
    
reset_syntax_error :-
  set_flag(last_syntax_error(_NoError,1000000)).
  
push_syntax_error(Message,OldFlag) -->
  remaining_chars(L),
  {push_flag(last_syntax_error,[message(Message),L],OldFlag)}.
  
pop_syntax_error(OldFlag) -->
  {pop_flag(last_syntax_error,OldFlag)}.
...
allowed_tablename(T,A) :-
  (datalog_metapredicate(T,A)
   ->
   my_raise_exception(unallowed_identifier(table,T),syntax(['']),[])
   ;
   true
  ).
...
delimited_sql_identifier(Connection,Identifier,StrDelimitedIdentifier) :-
  atom_codes(Identifier,StrIdentifier),
  my_odbc_get_dbms(Connection,DBMS),
  my_sql_left_quotation_mark(StrLeftDelim,DBMS),
  my_sql_right_quotation_mark(StrRightDelim,DBMS),
  concat_lists([StrLeftDelim,StrIdentifier,StrRightDelim],StrDelimitedIdentifier).
...
solve_rdb_sql_query(QueryStr,Query) :-
  ...
  !,
  translate_rdb_trusted_views(Query,QueryStr,TQueryStr,TSchema),
  object_to_trusted_object(Object,TSchema,TObject),
  my_odbc_ddl_query(TQueryStr),
  store_elapsed_time(computation),
...
rdb_ddl_message(Object,create,_Message,false,false,'View not created') :-
  ...
  !.
rdb_ddl_message(Object,drop,Message,true,false,Message) :-
  pdg((Nodes,_Arcs)),
  once((object_name_arity(Object,Name,Arity)
        ;
        object_name(Object,Name)
...
%   memberchk(UName/Arity,UNodes),
  ...
  !.
rdb_ddl_message(Object,drop,_Message,false,false,'Table not dropped') :-
  object_type(Object,table),
  !.
rdb_ddl_message(Object,drop,_Message,false,false,'View not dropped') :-
  object_type(Object,view),
  !.
rdb_ddl_message(Object,create_or_replace,CreateMessage,true,Existed,Message) :-
  once((object_name_arity(Object,Name,Arity)
        ;
        object_name(Object,Name)
      )),
...
my_guessed_ddl_statement('View created') -->
  my_complete_constrained_typed_schema(_,_),
  my_sql_blanks_star,
  ":=".
...
% Display RA compiled statements as SQL clauses
% From SQL:
display_compiled_ra(SQLst) :-
  language(ra),
  show_sql(on),
  !,
  write_log_list(['Info: Equivalent SQL query:',nl]),
  nl_compact_log,
  display_sql(SQLst,0),
  nl_compact_log.
...
solve_des_sql_query_k1(Lang,SQLst,Schema,ColTypes,TableRen,Query,DVs,RNVss,Undefined,OrderBy) :-
  ...
  replace_predicate_names_and_assert(Pred,Arity,answer,UDLsts,_DLsts,DVs,RNVss,ODLIds,Unsafe,Error),
...
%  DLsts = [Rule|_],
  ...
  (var(Error), is_setvar_safe(Unsafe)
   ->
...
%  sort_columns_by_relation_def(TableName,FAtts,OFAtts), 
  ...
  (my_primary_key('$des',FTableName,OFAtts) ->
   ...
   fail),
  sort_columns_by_relation_def(TableName,Atts,OAtts), 
  (my_foreign_key('$des',TableName,OAtts,FTableName,OFAtts) ->
...
post_table_constraint_assert(_TableName,my_integrity_constraint('$des',Preds,Constraint,NVs,Head,Ids,SQL,TableName),Check,Assert,_Error) :-
  ...
  (Assert==no_assert
   ...
   ;
    ...
    assert_rule((':-'(Head,Constraint),NVs),[],datalog,_,_,_,_,_),
...
check_ctr(my_types('$des',TableName,ColnameTypes),_CId) :-
  check_ic(on),
  !,
  exec_if_verbose_on(write_log_list(['Info: Checking types over database for relation ''',TableName,'''.',nl])),
  findall(Types,member(_Colname:Types,ColnameTypes),DeclaredTypes),
  length(ColnameTypes,Arity),
...
check_ctr(my_not_nullables('$des',TableName,NN_AttNames),CId) :-
  check_ic(on),
  !,
  exec_if_verbose_on(write_log_list(['Info: Checking nullability over database for relation ''',TableName,'''.',nl])),
  my_table('$des',TableName,Arity),
...
% Foreign Key
%
check_ctr(my_foreign_key('$des',TableName,FK_AttNames,FTableName,PK_AttNames),CId) :-
  check_ic(on),
  !,
  exec_if_verbose_on(write_log_list(['Info: Checking foreign key over database for relation ''',TableName,'''.',nl])),
  my_table('$des',TableName,Arity),
...
check_ctr(my_foreign_key('$des',_TableName,_FK_AttNames,_FTableName,_PK_AttNames),_CId).
...
check_ctr(my_functional_dependency('$des',TableName,AttNames,DepAttNames),CId) :-
  check_ic(on),
  !,
  exec_if_verbose_on(write_log_list(['Info: Checking functional dependency over database for relation ''',TableName,'''.',nl])),
  my_table('$des',TableName,Arity),
  functor(Fact,TableName,Arity),
...
check_ctr(my_unique_key('$des',TableName,AttNames),Kind,CId) :-
  ...
  exec_if_verbose_on(write_log_list(['Info: Checking ',Message,' over database for relation ''',TableName,'''.',nl])),
  my_table('$des',TableName,Arity),
  functor(Fact,TableName,Arity),
...
exist_relation(Connection,Relation) :-
  exist_relation(Connection,Relation,_Arity).
...
%   (functor(Head,Pred,Arity) 
%    ->
   replace_predicate_names_and_assert(Pred,Arity,TableName,DLsts,_RDLsts,DVs,CRNVs,ODLIds,Unsafe,_Error1),
...
%     logiql_output(create_view(TableName,TypedArgs,CRNVs)),
   ...
   ;
   create_view_k_error(Schema),
   my_raise_exception(generic,syntax(['Incorrect number of arguments (must be ',Arity,').']),[])
  ).
...
create_view_k_error(Schema) :-
  ...
  (retract(my_view('$des',TableName,_ArityV,_SQLst,_Lang,_DVs,_ODLIds,LocalViews,_SCs))
   ->
    drop_view_k_list([TableName|LocalViews])
   ;
...
drop_table(TableName,Warn,Warning,ClearET) :-
  ...
  my_retract_all_facts(my_foreign_key('$des',TableName,_PAtts,_FKTableName,_FKAtts)),
  my_retract_all_facts(my_foreign_key('$des',_TableName,_Atts,TableName,_FAtts)),
...
%  atom_codes(TableName,TableNameStr),
  my_odbc_relation_name(Connection,TableName,ODBCTableName),
...
%   reset_database,
%   update_stratification_remove_rules(_),
...
list_schema(Connection,Name) :-
  (opened_db(Connection)
   ->
    write_log_list(['Info: Database ''',Connection,'''',nl]),
...
write_integrity_constraint(my_integrity_constraint('$des',_Preds,Constraint,NVs,Head,Ids,SQL,_TableName),I) :-
    (development(off)
     ->
      (SQL==no_sql 
       ->
...
%   relation_arguments(AR1,Args),
%   union_schema(Args,RAS),
  arguments_autorenaming(RR,RAS).
...
sql_to_ra((select(DistinctAll,
  ...
  arguments_completion(SSProjList,_,RARelation,SSProjList,Ren2,Ren3,ProjList),
  arguments_completion(UOrderArgs,order,RARelation,SSProjList,Ren3,Ren4,OrderArgs),
  arguments_autorenaming(RR,ProjList), 
  expr_argument_completion(URACondition,RARelation,SSProjList,Ren4,ORen,RACondition),
...
arguments_completion_list([attr(Rel,A,_AS)|_As],_Scope,_Rel,_ProjList,_IRen,_ORen,_IPL,_OPL) :-
...
argument_completion(attr(Rel,A,AS),Scope,_RAst,PL,Ren,Ren,[expr(expr_ref(Rel,A),AS,_Type)]) :-
  ...
  !.
argument_completion(attr(Rel,A,AS),_Scope,RAst,_PL,Ren,Ren,[attr(Rel,A,AS)]) :-
...
argument_completion(attr(_Rel,A,AS),_Scope,_RAst,PL,Ren,Ren,[attr(R,C,AS)]) :-
  ...
  !.
argument_completion(expr(E,AS,Type),_Scope,RAst,PL,IRen,ORen,[expr(RE,AS,Type)]) :-
...
check_ambiguous_attrs(_A,As,R) :-
  ...
  !.
check_ambiguous_attrs(attr(_Rel,A,_AS),_L,_RA) :-
  my_raise_exception(generic,syntax(['Ambiguous column name ''',A,'''.']),[]).
...
rel_arguments(Rel,RAst,IRen,ORen,RAs) :-
  ...
  (RAs==[]
   -> 
     set_syntax_error(message(['Unknown relation name ''',Rel,''' in context ''',Rel,'.*".']),0),
     fail
%     my_raise_exception(generic,syntax(['Unknown relation name ''',Rel,''' in context ''',Rel,'.*''.']),[])
   ;
    true).
...
check_sql_condition(Scope,C) :-
  ...
  my_raise_exception(generic,syntax(['This aggregate is not allowed in the ',Clause,' clause: ','$exec'(write_expr(C,0))]),[]).
...
sql_join_cond_to_ra_cond(equijoin(natural),full_join,C,[LR|LArgs],[RR|RArgs],IProjList,OProjList,RCommonAtts,IRen,ORen) :-
  ...
  replace_common_atts_snn(TProjList,CommonAtts,RCommonAtts,IRen,ORen,OProjList).
...
replace_common_atts_snn(Atts,[],[],Ren,Ren,Atts).
replace_common_atts_snn([attr(R,N,A)|Atts],[attr(R,N,A)|CAtts],[attr(RId,c,V)|RCAtts],IRen,[(select_not_null,RId),Oren],[attr(RId,c,V)|RAtts]) :-
  visible_column_name(N,A,V),
  relation_autorenaming(RId),
  replace_common_atts_snn(Atts,CAtts,RCAtts,IRen,Oren,RAtts).
...
build_equijoin([attr(_RAtt,Att,_ArrRen)],[LAS|_LRArgs],[RAS|_RRArgs],ProjList,attr(LAS,Att,RA)=attr(RAS,Att,RB)) :-
  member(attr(LAS,Att,RA),ProjList),
  member(attr(RAS,Att,RB),ProjList).
...
%  simplify_arglist_expr(RArgs,SRArgs),
%   map_rel_id_var(RR,SRArgs,Args,Map1,OMap).
  build_mapping_from_schema_and_head_vars(RR,RArgs,Args,OMap).
...
cra_to_dl((pi(DistinctAll,TopN,ArgList,
  ...
  exprs_subgoals(order,OrderByList,RR,IsAggr,N2,_N21,Map2,Map3,Ren1,_Ren111,DLsts12,OBSGs),
  ...
  cond_subgoals(having,HavingCondition,N3,N4,Map1,_OMap,Ren1,_ORen,RSGs,DLsts3,HSGs),
...
rel_subgoals(times(RelA,B),N,LN,(SGA,As),DLsts,IMap,OMap,IRen,ORen) :-
  ...
  append(DLsts1,DLsts2,DLsts).
rel_subgoals((inner_join(RelA,RelB,C),_AS),N,LN,G,DLsts,IMap,OMap,IRen,ORen) :-
  !, 
...
rel_subgoals((left_join(RelA,RelB,C),_AS),N,LN,lj(SGA,SGB,SGC),DLsts,IMap,OMap,IRen,ORen) :-
  !, 
  rel_subgoals(RelA,N,N1,SGA,DLstsA,IMap,Map1,IRen,Ren1), 
  rel_subgoals(RelB,N1,N2,SGB,DLstsB,Map1,Map2,Ren1,Ren2), 
  on_subgoals(C,N2,LN,Map2,OMap,Ren2,ORen,DLstsC,SGC),
  concat_lists([DLstsA,DLstsB,DLstsC],DLsts).
rel_subgoals((right_join(RelA,RelB,C),_AS),N,LN,rj(SGA,SGB,SGC),DLsts,IMap,OMap,IRen,ORen) :-
  !, 
  rel_subgoals(RelA,N,N1,SGA,DLstsA,IMap,Map1,IRen,Ren1), 
  rel_subgoals(RelB,N1,N2,SGB,DLstsB,Map1,Map2,Ren1,Ren2), 
  on_subgoals(C,N2,LN,Map2,OMap,Ren2,ORen,DLstsC,SGC),
  concat_lists([DLstsA,DLstsB,DLstsC],DLsts).
rel_subgoals((full_join(RelA,RelB,C),_AS),N,LN,fj(SGA,SGB,SGC),DLsts,IMap,OMap,IRen,ORen) :-
  !, 
  rel_subgoals(RelA,N,N1,SGA,DLstsA,IMap,Map1,IRen,Ren1), 
  rel_subgoals(RelB,N1,N2,SGB,DLstsB,Map1,Map2,Ren1,Ren2), 
  on_subgoals(C,N2,LN,Map2,OMap,Ren2,ORen,DLstsC,SGC),
  concat_lists([DLstsA,DLstsB,DLstsC],DLsts).
% Natural full join:
rel_subgoals((full_join(RelA,RelB,C,Cs),_AS),N,LN,(fj(SGA,SGB,SGC),SNN),DLsts,IMap,OMap,IRen,ORen) :-
  !, 
  rel_subgoals(RelA,N,N1,SGA,DLstsA,IMap,Map1,IRen,Ren1), 
  rel_subgoals(RelB,N1,N2,SGB,DLstsB,Map1,Map2,Ren1,Ren2), 
  on_subgoals(C,N2,N3,Map2,Map3,Ren2,Ren3,DLstsC,SGC),
  nfj_on_subgoals(C,Cs,N3,LN,Map3,OMap,Ren3,ORen,SNN),
  concat_lists([DLstsA,DLstsB,DLstsC],DLsts).
rel_subgoals(Rel,N,LN,SG,DLsts,IMap,OMap,IRen,ORen) :-
  build_subgoal(Rel,N,LN,SG,DLsts,IMap,OMap,IRen,ORen).

build_subgoal((Table,[TableId|RArgs]),N,N,SG,[],IMap,OMap,IRen,ORen) :-
...
build_subgoal((pi(D,T,ArgList,R,G,H,O),[RR|RArgs]),N,LN,SG,DLsts,IMap,OMap,IRen,IRen) :-
  !,
  N1 is N+1,
...
% %   !.
map_col(_Scope,attr(Table,Col,ColRen),_Mapping,_Renaming,_Var) :-
  ((is_system_identifier(ColRen) ; var(ColRen)) -> ColName=Col ; ColName=ColRen),
  my_raise_exception(unknown_column(Table,ColName),syntax(''),[]).
...
nfj_on_subgoals(A1=A2,[attr(R,C,AS)],N,LN,IMap,OMap,IRen,ORen,select_not_null(V1,V2,V)) :-
  ...
  translate_expr(on,attr(R,C,AS),_IsAggr,N1,LN,[(V,R,C,AS)|Map1],Map2,[(select_not_null,R)|Ren1],ORen,[],V,_),
  replace_var_mapping_list([A1,A2],[V,V],Map2,OMap).
...
%  translate_cond(SOH,C2,N1,LN,Map1,OMap,Ren1,ORen,RelSGs,DLsts2,NC2),
  ...
  append(DLsts1,DLsts2,DLsts).
translate_cond(SOH,or(C1,C2),N,LN,IMap,OMap,IRen,IRen,RelSGs,DLsts,(NC1;NC2)) :-
...
% Unknown attribute
translate_expr(_Scope,attr(R,C,As),_IsAggr,_N,_LN,_IMap,_OMap,_IRen,_ORen,_DLsts,_DLE,_Goals) :- 
  (is_system_identifier(As) -> Col=C ; Col=As),
  my_raise_exception(unknown_column(R,Col),syntax(''),[]).
% Reference to an expression
translate_expr(_Scope,expr_ref(Rel,AS),_IsAggr,N,N,Map,Map,Ren,Ren,[],Var,true) :- % Use attribute if available
  member((Var,Rel,AS,_),Map),
  !.
translate_expr(_Scope,expr_ref(Rel,AS),_IsAggr,N,N,Map,Map,Ren,Ren,[],Var,true) :-
  !,
  member((Var,Rel,AS),Map).
translate_expr(_Scope,expr(expr_ref(Rel,AS),_,_),_IsAggr,N,N,Map,Map,Ren,Ren,[],Var,true) :-
  !,
  member((Var,Rel,AS),Map).
% DQL statement
...
%   - SQL Server
% MySQL
my_odbc_relation_name(Name,ODBCName) :-
  current_db(Connection),
  my_odbc_relation_name(Connection,Name,ODBCName).

my_odbc_relation_name(_Connection,Name,Name) :-
  var(Name),
  !.
my_odbc_relation_name(Connection,Name,ODBCName) :-
  opened_db(Connection,_Handle,DBMS),
  ...
  !,
  to_uppercase(Name,ODBCName).
% Some connectors tranlate relation names to lowercase
my_odbc_relation_name(Connection,Name,ODBCName) :-
  opened_db(Connection,_Handle,DBMS),
  memberchk(DBMS,[mysql,postgresql]),
  !,
  to_lowercase(Name,ODBCName).
my_odbc_relation_name(_Connection,Name,Name).
...
display_sql_show_sql_on(SQL) :-
  (show_sql(on) -> display_sql(SQL,0), nl_compact_log ; true).
...
write_sql_list([],_I).
write_sql_list([(not(SQLst),AS)],I) :-
  !,
  write_sql((SQLst,AS),I),
  write_log(' NOT IN'),
  pp_nl,
  I1 is I+2,
...
write_expr_ord_list([A],[O]) :-
  ...
  write_log(' '),
  write_ord(UO).
write_expr_ord_list([A1,A2|As],[O1,O2|Os]) :-
  write_expr_ord_list([A1],[O1]),
  write_log(', '),
  write_expr_ord_list([A2|As],[O2|Os]).
...
write_expr(expr(E,_AS,_Type),I) :-
  write_expr(E,I).
write_expr(cte('$NULL'(_ID),_Type),_I) :-
  !,
