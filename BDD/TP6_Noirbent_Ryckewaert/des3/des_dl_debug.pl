...
my_filter([], []).
my_filter([X|R1], [X|R2]) :- my_valid(X, true),!, my_filter(R1, R2).
my_filter([_X|R1], R2) :- my_filter(R1, R2).

my_valid([], true).
my_valid([(X,Y)|R], F ) :-  (incorrect_line((X,Y),R),!, F=false; my_valid(R, F)).
...
level_of_debugging([], _Goal, _Vars, _Level) :-
  write_log('Nothing for debugging'), !.
                     ...
                     (  size([pr,dup], Candidate, N),   %cuento los nodos de tipo Type que tienen estado sin asignar
                        ...
                        (  findNode(Type, N, Tree, Candidate, Node),     %busca el nodo (central)por el cual vamos a preguntar al usuario
                           ...
                           (    ([Op] == "n" ; [Op] == "N") ->
                                  ...
                                  update_state(Tree, Node, State), debug_strategy(Tree, Node, Vars, Type, New_list )
                                ;
                                (([Op] == "v" ; [Op] == "V") ->
...
dl_find_name_var(X, [Y=N|_RVars], N) :-
          X == Y, !.
dl_find_name_var(X, [_Y=_N|RVars], Name) :- dl_find_name_var(X, RVars, Name).
